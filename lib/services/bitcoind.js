...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of bitcore-gulden-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of bitcore-gulden-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of bitcore-gulden-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to GuldenD from bitcore. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that GuldenD is undergoing a reindex. ' +
             ...
             'of bitcore-gulden-node services will start.');
    node._reindex = true;
  }
};
